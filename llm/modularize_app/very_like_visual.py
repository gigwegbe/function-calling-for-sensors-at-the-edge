...
def auto_wrap_if_needed(cls, values):
    # Handle both missing 'readings' and empty dict case
    if "readings" not in values or not values.get("readings"):
        if any(isinstance(v, dict) for v in values.values()) or not values:
...
class VisualizationInput(BaseModel):
    ...
    @root_validator(pre=True)
    def auto_wrap_if_needed(cls, values):
        # If 'readings' is missing, assume the full input should go there
        if "readings" not in values and any(isinstance(v, dict) for v in values.values()):
            return {"readings": values}
        return values



@tool(args_schema=VisualizationInput)
def visualize_sensor_data(readings: Dict[str, Any]) -> str:
    """
    ...
    for sensor_name, data in readings.items():
        if "readings" in data and not data["readings"].get("error"):
            for metric, readings_list in data["readings"].items():
                ...
                print(f"DataFrame for {sensor_name} - {metric}:", df)
...
# Example input
input_query = "Get the plot of temperature from the south field today."

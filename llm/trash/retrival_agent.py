...
# Generalized function to get sensor data for a specific field
def get_sensor_data_for_field(field_name, sensor_type):
    ...
    if "sensor_list" not in llm_response or not llm_response["sensor_list"]:
        return {"error": f"No sensors found for the specified field: {field_name}."}
    ...
    if sensor_type not in device_keys:
        return {"error": f"Sensor type '{sensor_type}' not found in device keys."}

    end_ts = int(time.time() * 1000)
    start_ts = end_ts - (24 * 60 * 60 * 1000)  # Last 24 hours

    historical_data = get_historical_data(jwt_token, device_id, start_ts, end_ts, sensor_type)
    return historical_data

# Example usage
field_name = "North Field"
sensor_type = "humidity"  # Replace with any sensor type, e.g., "humidity", "pressure"
sensor_data = get_sensor_data_for_field(field_name, sensor_type)
print(f"{sensor_type.capitalize()} data for {field_name}: {sensor_data}")
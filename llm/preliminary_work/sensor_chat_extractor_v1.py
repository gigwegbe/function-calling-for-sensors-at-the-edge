...
# def build_sensor_extraction_tool(model, prompt_template):
#     runnable = model.with_structured_output(SensorExtraction).bind(prompt=prompt_template)

#     @tool
#     def sensor_extraction(input: str) -> dict:
#         """Extract relevant sensor IDs from a user query about farm fields."""
#         # ✅ input must be passed as a string
...
def build_sensor_extraction_tool(model, prompt_template):
    ...
    def sensor_extraction(input: str) -> dict:
        """Extract relevant sensor IDs from a user query about farm fields."""
        # Pass input directly to the model's prompt
        response = model.invoke(input=prompt_template.format(input=input))
...
input_query = "Get all humidity and temperature reading in the field today."

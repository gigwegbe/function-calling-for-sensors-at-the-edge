...
#     cb = cl.LangchainCallbackHandler()
#     final_answer = cl.Message(content="")
#     print(f"User message received: {message.content}")
...
#         print(f"Supervisor Output Step: {output}")
#         if isinstance(output, dict):
#             if "supervisor" in output and "messages" in output["supervisor"] and output["supervisor"]["messages"]:
#                 last_supervisor_message = output["supervisor"]["messages"][-1]
#                 if isinstance(last_supervisor_message, AIMessage):
#                     print(f"Found final supervisor AIMessage: {last_supervisor_message.content}")
#                     await final_answer.stream_token(last_supervisor_message.content)
#             elif "messages" in output and output["messages"]:
#                 # last_message = output["messages"][-2]
#                 last_message = output["messages"]
#                 if isinstance(last_message, AIMessage):
#                     print(f"Found direct AIMessage: {last_message.content}")
...
#     cb = cl.LangchainCallbackHandler()
#     final_answer = cl.Message(content="")
#     plot_filename = None  # Store the filename if the agent mentions it
...
#                     if isinstance(agent_message, AIMessage) and "plot.png" in agent_message.content:
#                         plot_filename = "plot.png"
#                         await cl.Message(content="Here is the sensor plot:").send() # Send the message first
#                         try:
#                             with open(plot_filename, "rb") as f:
#                                 content = f.read()
#                             await cl.Message(
...
#                     print(f"Found direct AIMessage: {last_message.content}")
#                     await final_answer.stream_token(last_message.content)
#                 elif isinstance(last_message.content, str) and last_message.content == "plot.png":
#                     try:
#                         with open("plot.png", "rb") as f:
#                             content = f.read()
#                         await cl.Message(
...
async def on_message(message: cl.Message):
    ...
    async for output in top_supervisor.astream(
        ...
        if isinstance(output, dict):
            if "supervisor" in output and "messages" in output["supervisor"] and output["supervisor"]["messages"]:
                ...
                last_message = output["messages"][-1]  # Get the last message in the list
                if isinstance(last_message, AIMessage):
                    print(f"Found direct AIMessage: {last_message.content}")
                    await final_answer.stream_token(last_message.content)
                elif isinstance(last_message.content, str) and last_message.content == "plot.png":
                    # If the tool returned the filename, send it as an image
                    await cl.Image(name="Sensor Data Plot", path="plot.png").send()
                    return  # Important: Exit the function after sending the image
        else:
            print(f"Output is not a dict: {output}")
    ...
    await final_answer.send()
        ...
        #             name="Sensor Plot",
        #             display="inline",
        #             id=str(uuid.uuid4()),       # required in older versions too
        #             content=content
        #         )
        #     ]
        # ).send()

...
def fetch_telemetry(device_id, token, keys="temp,moisture_content,relative_humidity,soil_conductivity", start_ts=None, end_ts=None, interval=1800000):
...
def fetch_sensor_telemetry(input: TelemetryRequest) -> dict:
    ...
    # Process devices in parallel if there are multiple devices
    for name in input.device_names:
        try:
            device_id = get_device_id_by_name(name, token)
            data = fetch_telemetry(
                ...
                start_ts=start_ts,
                end_ts=end_ts,
                # Increase interval to reduce data points (3600000 = 1 hour)
                interval=3600000
            )

            # Process raw data with optimized code
            processed_data = {}
            for key, values in data.items():
                processed_data[key] = [
                    {
                        "timestamp": convert_timestamp_to_readable(reading['ts']),
                        "value": reading['value']
                    }
                    for reading in values
                ]
...
def visualize_sensor_data(data: dict) -> str:
    ...
    # Optimize subplots creation
    fig = make_subplots(rows=num_sensors, cols=1, shared_xaxes=True, vertical_spacing=0.1,
                         subplot_titles=[sensor_id for sensor_id in data.keys()])
    
    row = 1
    for sensor_id, sensor_metrics in data.items():
        for metric_name, readings in sensor_metrics.items():
            # Process only if there's actual data
            if not readings:
                continue
                
            # Limit data points for faster rendering (sample every N points)
            sample_rate = max(1, len(readings) // 50)  # Max 50 points per series
            
            # Use list comprehension for faster data preparation
            x = [datetime.strptime(reading['timestamp'], '%Y-%m-%d %H:%M:%S') 
                 for reading in readings[::sample_rate]]
            y = [float(reading['value']) for reading in readings[::sample_rate]]
            
            if x and y:
                # Use Scatter instead of ScatterGL for simpler plots
                trace = go.Scatter(x=x, y=y, mode='lines', name=f"{sensor_id} - {metric_name}")
                fig.add_trace(trace, row=row, col=1)
    ...
    fig.update_layout(
        ...
        xaxis_title="Time",
        height=300*num_sensors,  # Adjust height based on number of sensors
        template="plotly",  # Use simpler template
        showlegend=True
    )

    # Optimize image writing
    filename = "plot.png"
    fig.write_image(filename, engine="kaleido", scale=1)
    return filename
...
# --- Optimize on_message handler ---
@cl.on_message
async def on_message(message: cl.Message):
    config = {"configurable": {"thread_id": cl.context.session.id}}
    cb = cl.LangchainCallbackHandler()
    
    # Track final response and plot status
    final_response = ""
    plot_file = "plot.png"
    plot_sent = False
    
    # Process message - use a timeout to prevent hanging
    try:
        async for output in top_supervisor.astream(
            {"messages": [HumanMessage(content=message.content)]},
            config=RunnableConfig(callbacks=[cb], **config)
            ...
            if "sensor_visualization_agent" in output:
                ...
                if agent_output and "messages" in agent_output:
                    for msg in agent_output["messages"]:
                        if isinstance(msg, AIMessage) and plot_file in msg.content:
                            # Faster file checking
                            if os.path.exists(plot_file):
                                # Cleaner regex for explanation extraction
                                explanation = re.sub(r'!\[.*?\]\(.*?\)|!\[.*?\]\(.*', '', msg.content).strip()
                                if not explanation:
                                    explanation = "Here is the sensor data visualization:"
                                ...
                                plot_sent = True
                                break  # Exit after finding the first plot
            ...
            if "supervisor" in output:
                ...
                if supervisor_output and "messages" in supervisor_output:
                    for msg in supervisor_output["messages"]:
                        if isinstance(msg, AIMessage):
                            final_response = re.sub(r'!\[.*?\]\(.*?\)|!\[.*?\]\(.*', '', msg.content).strip()
        
        # Send text response if no plot was sent
        if not plot_sent and final_response:
            await cl.Message(content=final_response).send()

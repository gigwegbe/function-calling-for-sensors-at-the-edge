...
async def on_message(message: cl.Message):
    ...
    try:
        ...
        try:
            ...
            # Check if supervisor delegated to an agent
            if "agent_outcome" in raw_result:
                if isinstance(raw_result["agent_outcome"], dict) and "output" in raw_result["agent_outcome"]:
                    await thinking.update(content=raw_result["agent_outcome"]["output"])
                    return
                # Some versions might return the entire agent response object
                elif hasattr(raw_result["agent_outcome"], "return_values") and hasattr(raw_result["agent_outcome"].return_values, "get"):
                    output = raw_result["agent_outcome"].return_values.get("output")
                    if output:
                        await thinking.update(content=output)
                        return
            
            # Check if there's a direct output
            if "output" in raw_result:
                await thinking.update(content=raw_result["output"])
                return
                
            # If supervisor sent a message directly
            if "messages" in raw_result and raw_result["messages"]:
                supervisor_message = raw_result["messages"][0].content
                
                # Detect if this is just a greeting or clarification request
                if any(phrase in supervisor_message.lower() for phrase in ["how can i assist", "help you", "provide details"]):
                    # The supervisor is not routing - we'll handle directly
                    print("Supervisor is not routing query, handling directly...")
                else:
                    # Pass through the supervisor message
                    await thinking.update(content=supervisor_message)
                    return
        ...
        try:
            if "temperature" in query or "temp" in query:
                field_part = query.split("temperature")[1] if "temperature" in query else query.split("temp")[1]
                ...
                if isinstance(result, dict) and "sensor_id" in result:
                    ...
                    if telemetry_result:
                        await thinking.update(content=f"Here's the temperature data for your query:\n\n```json\n{json.dumps(telemetry_result, indent=2)}\n```")
                        ...
                        await thinking.update(content="I couldn't find temperature data for that query. Please check if the sensors are online.")
                else:
                    await thinking.update(content="I couldn't identify which sensors to query. Please specify the field more clearly.")
                return
                ...
                if isinstance(result, dict) and "sensor_id" in result:
                    ...
                    if telemetry_result:
                        await thinking.update(content=f"Here's the humidity data for your query:\n\n```json\n{json.dumps(telemetry_result, indent=2)}\n```")
                        ...
                        await thinking.update(content="I couldn't find humidity data for that query. Please check if the sensors are online.")
                else:
                    await thinking.update(content="I couldn't identify which sensors to query. Please specify the field more clearly.")
                return
                ...
                if isinstance(result, dict) and "sensor_id" in result:
                    ...
                    if telemetry_result:
                        await thinking.update(content=f"Here's the sensor data for your query:\n\n```json\n{json.dumps(telemetry_result, indent=2)}\n```")
                        ...
                        await thinking.update(content="I couldn't find sensor data for that query. Please check if the sensors are online.")
                else:
                    await thinking.update(content="I couldn't identify which sensors to query. Please specify what you're looking for more clearly.")
                return
                
        except Exception as direct_error:
            print(f"Direct handling error: {direct_error}")
            await thinking.update(content=f"I had trouble processing your request. Please try again with more details about which field or sensor you're interested in.")
            return
        
        # Final fallback
        await thinking.update(content="I processed your request but couldn't get the right data. Could you try being more specific about which sensors or fields you're interested in?")

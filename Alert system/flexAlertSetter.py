...
def build_temperature_rule_chain(sensor_field):
    rule_chain_data = {
...
def main():
    ...
    print("Available sensors: soil moisture, soil temperature, electro-conductivity, humidity")
    sensor_field = input("Enter the sensor field (e.g., soil moisture): ").strip().lower().replace(" ", "_")
    ...
    # Step 2: Build rule chain dynamically
    rule_chain_data = build_temperature_rule_chain(sensor_field)
    ...
    if created_rule_chain:
        ...
        if rule_chain_id:
            metadata = build_rule_chain_metadata(rule_chain_id, sensor_field, threshold_value)
